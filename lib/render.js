...
function updateDataTable() {
  ...
  el.innerHTML = `<table><thead><tr><th>ID</th><th>W</th>${dpCols.map(col => `<th>${col.name}</th>`).join('')}</tr></thead><tbody>${rows.map(n => `<tr><td style="color:var(--accent)">${n.id}</td><td>${n.weight}</td>${dpCols.map(col => `<td style="color:var(--purple)">${formatVal(state.dpResults[n.id]?.[col.target || col.name])}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
}
...
function showSelInfo() {
  ...
  if (count > 1) {
    ...
    const ws = nodes.map(n => n.weight);
    const wInput = document.getElementById('selWeight');
    const allSame = ws.length > 0 && ws.every(w => w === ws[0]);
    wInput.value = allSame ? ws[0] : ''; wInput.placeholder = allSame ? '' : 'mixed';
  ...
  const wInput = document.getElementById('selWeight');
  wInput.value = obj.weight; wInput.placeholder = '';
  if (isNode) {
  ...
  } else { xyGroup.classList.add('hidden'); }
