...
// Editor-only settings (kept separate from `state` object per design).
const editorSettings = { editorKeyMode: 'common' };
...
function makeSnapshot() {
  ...
  // include editor-specific settings with the snapshot so snapshots
  // and tree lists carry the editor configuration alongside the tree
  snap.editorSettings = editorSettings;
  return snap;
}
...
function loadSnapshot(snap) {
  ...
  // Restore editorSettings if present in the snapshot
  if (snap && snap.editorSettings) {
    window.editorSettings = snap.editorSettings;
  }
}
...
function loadFromStorage() {
  try {
    ...
    if (data.treeSnapshots && Array.isArray(data.treeSnapshots)) {
      ...
      // restore top-level editorSettings if present (for older saves this may be undefined)
      if (data.editorSettings) window.editorSettings = data.editorSettings;
      return true;
    } else {
      ...
      document.getElementById('dpWarning').classList.toggle('hidden', state.isTree);
      quickParseDpDefs(data.dpCode);
      // restore top-level editorSettings if present
      if (data.editorSettings) window.editorSettings = data.editorSettings;
      return true;
    }
...
const saveHistory = () => {
  ...
  hist.stack.push(JSON.stringify({ nodes: state.nodes, edges: state.edges, nextId: state.nextId, editorSettings: editorSettings }));
  hist.idx++;
...
const undo = () => {
  if (hist.idx > 0) {
    ...
    state.selected = null; state.multiSelected = new Set(); state.dpResults = {};
    // restore editorSettings from history entry if present
    if (snap.editorSettings) window.editorSettings = snap.editorSettings;
    fullUpdate();
  }
};
const redo = () => {
