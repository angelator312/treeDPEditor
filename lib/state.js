...
function saveToStorage() {
  try {
    ...
    if (window.treeSnapshots && window.treeSnapshots.length > 0) {
      ...
      window.treeSnapshots[window.currentTreeIndex] = JSON.parse(JSON.stringify(state));
...
function loadFromStorage() {
  try {
    ...
    if (data.treeSnapshots && Array.isArray(data.treeSnapshots)) {
      window.treeSnapshots = data.treeSnapshots;
      window.currentTreeIndex = typeof data.currentTreeIndex === 'number' ? data.currentTreeIndex : 0;
      // Load selected tree into state
      Object.assign(state, JSON.parse(JSON.stringify(window.treeSnapshots[window.currentTreeIndex] || {})));
