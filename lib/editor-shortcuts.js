...
(function(){
  ...
  function moveBlock(textarea, dir /* 'up'|'down' */) {
    ...
    if (dir === 'up') {
      ...
      saveHistory();
      textarea.value = beforePrev + curBlock + prevBlock + afterCur;
      ...
      saveHistory();
      textarea.value = before + nextBlock + curBlock + afterNext;
      const newStart = before.length + nextBlock.length;
      textarea.selectionStart = newStart;
      textarea.selectionEnd = newStart + curBlock.length;
      return true;
    }
