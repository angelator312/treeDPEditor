...
function bindEvents() {
  ...
  // Initialize editorKeyMode control if present and bind change handler
  const editorModeEl = document.getElementById('editorKeyMode');
  if (editorModeEl) {
    // set initial value from loaded settings (loadFromStorage runs before bindEvents)
    editorModeEl.value = (window.editorSettings && window.editorSettings.editorKeyMode) ? window.editorSettings.editorKeyMode : 'common';
    editorModeEl.onchange = e => {
      // Changing editor mode is considered a user action we want undoable
      saveHistory();
      window.editorSettings = window.editorSettings || {};
      window.editorSettings.editorKeyMode = e.target.value;
      saveToStorage();
    };
  }
  ...
  // Excalidraw export controls
  const btnCopyExcal = document.getElementById('btnCopyExcalidraw');
  const btnDownloadExcal = document.getElementById('btnDownloadExcalidraw');
  const tglExportLabels = document.getElementById('tglExportLabels');
  if (btnCopyExcal) {
    btnCopyExcal.onclick = async () => {
      if (!window.treeSnapshots || window.treeSnapshots.length === 0) { alert('No tree to export.'); return; }
      const idx = (typeof window.currentTreeIndex === 'number') ? window.currentTreeIndex : 0;
      const tree = window.treeSnapshots[idx];
      if (!tree) { alert('No tree selected.'); return; }
      const includeLabels = tglExportLabels ? !!tglExportLabels.checked : true;
      const elements = window.exportSnapshotToExcalidraw(tree, { scale: 1, includeLabels });
      const ok = await window.copyElementsToClipboard(elements);
      if (ok) { btnCopyExcal.textContent = 'Done'; setTimeout(() => btnCopyExcal.textContent = 'Copy Excalidraw Elements', 1000); }
      else alert('Copy to clipboard failed — allow clipboard permission or use Download.');
    };
  }
  if (btnDownloadExcal) {
    btnDownloadExcal.onclick = () => {
      if (!window.treeSnapshots || window.treeSnapshots.length === 0) { alert('No tree to export.'); return; }
      const idx = (typeof window.currentTreeIndex === 'number') ? window.currentTreeIndex : 0;
      const tree = window.treeSnapshots[idx];
      if (!tree) { alert('No tree selected.'); return; }
      const includeLabels = tglExportLabels ? !!tglExportLabels.checked : true;
      const elements = window.exportSnapshotToExcalidraw(tree, { scale: 1, includeLabels });
      window.downloadElementsJSON(elements, (tree.name || 'tree') + '.excalidraw.json');
    };
  }
  ...
  // Attach editor shortcuts (defined in lib/editor-shortcuts.js)
  if (typeof setupEditorShortcuts === 'function') {
    const dpCodeEl = document.getElementById('dpCode');
    try { setupEditorShortcuts(dpCodeEl); } catch (e) { console.error('Failed to setup editor shortcuts', e); }
  }
