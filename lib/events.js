...
function renderTreesList() {
  ...
  treeSnapshots.forEach((snap, idx) => {
    ...
    div.innerHTML = `
      ...
      <button class="btn-switch-tree btn text-xs px-2 py-0.5 rounded" data-idx="${idx}" style="background: var(--accent); color: var(--bg);">Switch</button>
  ...
  list.querySelectorAll('.btn-switch-tree').forEach(btn => {
    btn.onclick = e => {
      ...
      if (idx !== currentTreeIndex) {
        // Save current state to snapshot before switching
        if (treeSnapshots[currentTreeIndex]) {
          treeSnapshots[currentTreeIndex] = JSON.parse(JSON.stringify(state));
        }
        // Load selected snapshot into state
        Object.assign(state, JSON.parse(JSON.stringify(treeSnapshots[idx])));
  ...
  list.querySelectorAll('.btn-del-tree').forEach(btn => {
    btn.onclick = e => {
      const idx = +btn.dataset.idx;
      if (treeSnapshots.length <= 1) return; // Prevent deleting last tree
      treeSnapshots.splice(idx, 1);
      if (currentTreeIndex >= treeSnapshots.length) currentTreeIndex = treeSnapshots.length - 1;
      // Load new current tree
      Object.assign(state, JSON.parse(JSON.stringify(treeSnapshots[currentTreeIndex])));
      render();
      renderTreesList();
    };
  });
...
function bindEvents() {
                ...
                importNewTreeInput.onchange = e => {
                  ...
                  reader.onload = evt => {
                    try {
                      ...
                      // Give a default name if missing
      ...
      if (btnBatchDP) {
        btnBatchDP.onclick = async () => {
          ...
          for (const idx of selected) {
            ...
            results.push(`<b>${treeSnapshots[idx].name||('Tree '+(idx+1))}:</b><br><pre>${output}</pre>`);
          }
    ...
    if (btnAddTree) {
      btnAddTree.onclick = () => {
        // Deep copy current state (shallow for now)
